...
const { evaluate, sandboxFor } = await import('./evaluate');
...
const fakeSandbox = { name: 'fake' } as never;
...
test("a code grader's own floor is insufficient, decided after collection succeeded", async () => {
  code.runCodeGrader.mockResolvedValue({
    result: { ...scoredResult, score: null },
    insufficient: true,
  });
  const evaluation = await evaluate(
    codeManifest,
  ...
  expect(evaluation.verdict).toBe('insufficient');
  expect(code.runCodeGrader).toHaveBeenCalledWith(codeManifest, expect.objectContaining({ sha }), {
    name: 'fake',
  });
});
...
test('a code grader that answered is scored', async () => {
  ...
  // The caller chose the box; evaluation never selects one of its own.
  expect(sandbox.selectSandbox).not.toHaveBeenCalled();
});

test('a code grader is handed a sandbox chosen before anything is collected', () => {
  expect(sandboxFor(codeManifest)).toEqual({ name: 'fake' });
  expect(sandbox.selectSandbox).toHaveBeenCalledTimes(1);
});

test('a declarative grader never asks for a sandbox', () => {
  expect(sandboxFor(agentReadinessManifest)).toBeNull();
  expect(sandbox.selectSandbox).not.toHaveBeenCalled();
});
